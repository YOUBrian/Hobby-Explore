...
class ChatGptAdapter(private val messageList: List<ChatGPTMessage>) :
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return when (message.sentBy) {
            ChatGPTMessage.SENT_BY_ME -> SENT_BY_ME
            else -> SENT_BY_BOT
        }
    }
    ...
    inner class MessageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(message: ChatGPTMessage) {
            when (message.sentBy) {
                ChatGPTMessage.SENT_BY_ME -> {
